...
class Comics with ChangeNotifier {
  final List<Comic> _items = [
    ...
    //     name: 'Ветролом',
    //     description:
    //         'Ветролом — драма о юных уличных гонщиках, мечтающих о свободе. Ветролом — драма о юных уличных гонщиках, мечтающих о свободе. Ветролом — драма о юных уличных гонщиках, мечтающих о свободе. Ветролом — драма о юных уличных гонщиках, мечтающих о свободе. Ветролом — драма о юных уличных гонщиках, мечтающих о свободе. Ветролом — драма о юных уличных гонщиках, мечтающих о свободе.',
    //     imageUrl:
  ...
  Future<void> getComics() async {
    ...
    final allData =
        querySnapshot.docs.map((doc) => Comic.fromSnapshot(doc)).toList();
    allData.forEach(
      (element) => _items.add(element),
    );
    notifyListeners();
  }
  ...
  void toggleFav(String id) {
    ...
    Comic newCom = Comic(
        id: oldCom.id,
        rusName: oldCom.rusName,
        engName: oldCom.engName,
        rusDescription: oldCom.rusDescription,
        engDescription: oldCom.engDescription,
